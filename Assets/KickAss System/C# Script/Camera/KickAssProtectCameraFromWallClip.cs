...
public class KickAssProtectCameraFromWallClip : MonoBehaviour {

	public bool IsVR = false;
	public float clipMoveTime = 0.05f;              // time taken to move when avoiding cliping (low itemValue = fast, which it should be)
	public float returnTime = 0.4f;                 // time taken to move back towards desired position, when not clipping (typically should be a higher itemValue than clipMoveTime)
	...
	private void Start()
	{
		...
		if(IsVR == true){
			m_Cam = GameObject.FindGameObjectWithTag("MainCamera").GetComponent<Transform>();
	...
	private void LateUpdate()
	{
		if(m_Cam == null){
			if(IsVR == true){
				m_Cam = GameObject.Find("PivotCam").GetComponent<Transform>();
				m_Pivot = GameObject.Find("Pivot").GetComponent<Transform>();
			}else{
				m_Cam = GetComponentInChildren<Camera>().transform;
				m_Pivot = m_Cam.parent;
			}
